...
const cases = [
  ...
  {
    id: 'case-4',
    title: 'Платформа персонализации E-commerce',
    category: 'E-commerce / Retail',
    tech: ['Next.js', 'Elasticsearch', 'FastAPI'],
    problem: 'Низкая конверсия в рекомендательных блоках корзины и карточки товара. ' +
             'Существующая система базировалась на простых эвристиках, не учитывая поведение отдельного пользователя.',
    solution: 'Разработан и внедрен рекомендательный движок на базе ML. Спроектировано API на FastAPI для быстрой отдачи предсказаний, ' +
              'интегрирован Elasticsearch для гибридного поиска (семантика + фильтры).',
    result: 'Рост конверсии (CR) в рекомендательных блоках на 22%. Увеличение среднего чека (AOV) на 14% за счет релевантного кросс-селла ' +
            'и динамического подбора сопутствующих товаров.'
  },
  {
    id: 'case-5',
    title: 'Аналитика обратной связи',
    category: 'Data Analytics / NLP',
    tech: ['Python', 'Transformers', 'ClickHouse'],
    problem: 'Тысячи неструктурированных отзывов и тикетов копились ежемесячно. Маркетинг и отдел разработки ' +
             'не имели возможности оперативно выявлять тренды негатива и баги после релизов.',
    solution: 'Построена архитектура NLP-пайплайна для автоматического тегирования тональности (Sentiment) и тематики отзывов. ' +
              'Данные складываются в ClickHouse для аналитики в реальном времени, визуализация выведена в дашборды.',
    result: 'Скорость реакции на критические продуктовые баги снизилась с 2 дней до 3 часов. ' +
            'Аналитики получили автоматизированную систему расчета CSI (Customer Satisfaction Index).'
  },
  {
    id: 'case-6',
    title: 'Миграция и оркестрация',
    category: 'Infrastructure / DevOps',
    tech: ['Kubernetes', 'Terraform', 'GitLab CI'],
    problem: 'Предыдущая монолитная инфраструктура на VM не справлялась с сезонными пиковыми нагрузками. ' +
             'Релизы требовали даунтайма, масштабирование занимало часы.',
    solution: 'Инфраструктура описана как код (IaC) с помощью Terraform. Осуществлена контейнеризация приложений и ' +
              'поэтапный перенос трафика в K8s-кластер. Настроено автоматическое масштабирование (HPA).',
    result: 'Zero Downtime деплой позволил релизить фичи днем без перерыва в обслуживании клиентов. Снижение расходов на инфраструктуру на 30% ' +
            'за счет утилизации ресурсов и автомасштабирования в ночные часы.'
  }
];
